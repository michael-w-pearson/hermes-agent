...
class GatewayTopicThreadsMixin:
    ...
    def _recover_telegram_topic_thread_id(self, source: SessionSource) -> Optional[str]:
        ...
        # Operator-reserved DM lanes are outbound-only surfaces. A human reply
        # that arrives there must enter the configured conversation lane before
        # session keying; explicit cron/status deliveries never traverse this
        # inbound recovery hook.
        telegram_cfg = getattr(getattr(self, "config", None), "platforms", {}).get(
            Platform.TELEGRAM
        )
        extra = getattr(telegram_cfg, "extra", None) or {}
        ...
        if isinstance(dm_topics, list):
            configured_chat = any(
                ...
                and str(entry.get("chat_id") or "") == str(source.chat_id)
                for entry in dm_topics
            )
        if configured_chat and isinstance(policy, dict):
            conversation_thread = str(
                policy.get("default_conversation_thread_id") or ""
            )
            reserved_threads = {
                str(policy.get(key) or "")
                for key in ("report_thread_id", "error_thread_id")
            }
            reserved_threads.discard("")
            if (
                conversation_thread
                and inbound in reserved_threads
                and inbound != conversation_thread
            ):
