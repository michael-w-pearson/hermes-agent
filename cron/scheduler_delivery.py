...
def _delivery_lane_value(job: dict, *, for_failure: bool = False):
    ...
    if for_failure:
        ...
        try:
            cfg = _sched.load_config() or {}
            global_failure_deliver = str(
                ((cfg.get("cron") or {}).get("error_delivery_target") or "")
            ).strip()
        except Exception:
            # Fail closed without raising: delivery routing is not allowed to wedge
            # mark_job_run()/finish_execution(), and exception chaining would leak
            # parser/config details through the scheduler's traceback logger.
            logger.error(FAILURE_LANE_CONFIG_ERROR)
            return "local"
        if global_failure_deliver:
            return global_failure_deliver
